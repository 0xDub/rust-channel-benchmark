...
def analyze_inter_async(sender_threshold, receiver_threshold):
    ...
    methods = ["flume", "kanal", "tokio", "async_channel", "crossfire", "thingbuf", "tachyonix", "postage", "async_broadcast"] #

    aggregate_data = pd.DataFrame(columns=["latency", "method"])

    print()
    print(bcolors.OKCYAN + "=-= Inter-Process (Async) | Latency Stats (ns) =-=" + bcolors.ENDC)
    ...
    for method in methods:
        ...
        sender_ax.hist(sender_data["latency"], bins=bins, alpha=alpha, density=True, color=color, label=method)
        sender_ax.axvline(sender_data["latency"].mean(), color=color, linestyle='solid', linewidth=1)
        receiver_ax.hist(recv_data["latency"], bins=bins, alpha=alpha, density=True, color=color, label=method)
        receiver_ax.axvline(recv_data["latency"].mean(), color=color, linestyle='solid', linewidth=1)
...
def analyze_inter_busy(sender_threshold, receiver_threshold):
    ...
    methods = ["rtrb", "ringbuf", "crossbeam_channel", "crossbeam_queue", "kanal", "flume", "omango", "npnc", "magnetic", "tokio"] #

    aggregate_data = pd.DataFrame(columns=["latency", "method"])

    print()
    print(bcolors.OKCYAN + "=-= Inter-Process (Busy) | Latency Stats (ns) =-=" + bcolors.ENDC)
...
def analyze_intra_async_single_thread(sender_threshold, receiver_threshold):
    ...
    print()
    print(bcolors.OKCYAN + "=-= Intra-Process (Async Single Thread) | Latency Stats (ns) =-=" + bcolors.ENDC)

    fig, sender_ax = plt.subplots(1)
    sender_ax.set_title("Intra-Process (Async Single Thread) | Sender | Latency Distribution")
    sender_ax.set_xlabel("Latency (ns)")
    sender_ax.set_ylabel("Density")

    fig, receiver_ax = plt.subplots(1)
    receiver_ax.set_title("Intra-Process (Async Single Thread) | Receiver | Latency Distribution")
    receiver_ax.set_xlabel("Latency (ns)")
    receiver_ax.set_ylabel("Density")
    ...
    for method in methods:
        ...
        if len(aggregate_data) == 0:
            ...
            aggregate_data = pd.concat([aggregate_data, sender_data])
        # =------------------------------------------------------------= #
        with open(f"intra_latencies/async_single_thread/{method}_receiver.txt", "r") as f:
            lines = f.readlines()
            lines = [x.rstrip() for x in lines]
            lines = [int(x) for x in lines if x != ""]
        recv_data = pd.DataFrame(lines, columns=["latency"])
...
def analyze_intra_async_multi_thread(sender_threshold, receiver_threshold):
    ...
    print()
    print(bcolors.OKCYAN + "=-= Intra-Process (Async Multi Thread) | Latency Stats (ns) =-=" + bcolors.ENDC)

    fig, sender_ax = plt.subplots(1)
    sender_ax.set_title("Intra-Process (Async Multi Thread) | Sender | Latency Distribution")
    sender_ax.set_xlabel("Latency (ns)")
    sender_ax.set_ylabel("Density")

    fig, receiver_ax = plt.subplots(1)
    receiver_ax.set_title("Intra-Process (Async Multi Thread) | Receiver | Latency Distribution")
    receiver_ax.set_xlabel("Latency (ns)")
    receiver_ax.set_ylabel("Density")
    ...
    for method in methods:
        ...
        if len(aggregate_data) == 0:
            ...
            aggregate_data = pd.concat([aggregate_data, sender_data])
        # =------------------------------------------------------------= #
        with open(f"intra_latencies/async_multi_thread/{method}_receiver.txt", "r") as f:
            lines = f.readlines()
            lines = [x.rstrip() for x in lines]
            lines = [int(x) for x in lines if x != ""]
        recv_data = pd.DataFrame(lines, columns=["latency"])
